...
// タスク
// ---------------------------------------
var Task = Class(LinkedList.Node,{
    // コンストラクタ
    initialize : function(
        nowTick,        // 登録時間
        nextTick,       // 実行時間
    ...
    add : function(nowTick,nextTick,callback){
        ...
        var ret = this.taskList_.scanTail(function(node){
            ...
            if(execTime > existTime){
                // 見つけたら直前に追加する
                this.taskList_.afterInsert(node,task);
                return false;
